...
<template>
  <div class="col">
    <div class="card flush">
      <div class="toolbar">
        <SearchInput :value="''" @change="() => {}" placeholder="Search EPG sources" />
        <span class="spacer" />
        <Btn variant="ghost" icon="refresh">Sync all</Btn>
        <Btn variant="primary" icon="plus" @click="emit('add', 'epg')">Add EPG source</Btn>
      </div>
      <div v-for="p in EPG_SOURCES" :key="p.id" class="src-row" @click="router.push(`/epg-sources/${p.id}`)">
